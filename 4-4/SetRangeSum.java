...
public class SetRangeSum {
    ...
    void insert(int x) {
        ...
        */
        Vertex curr = root;
        if(curr == null) {
            root = new Vertex(x, x, null, null, null);
            return;
        }
        while(curr != null){
            if(curr.key == x){
            ...
            if(curr.key > x){
                if(curr.left != null) curr = curr.left;
                    ...
                    curr.left = new Vertex(x, x, null, null, curr);
                    update(curr);
                    root = splay(curr.left);
                    break;
                }
            }
            else{
                if(curr.right != null) curr = curr.right;
                    ...
                    curr.right = new Vertex(x,x,null,null,curr);
                    update(curr);
                    root = splay(curr.right);
                    break;
                }
            }
        }
    ...
    void erase(int x) {
        ...
        update(root);
        if(result.left == null || result.left.key != x) return;
        else{
            if(result.right.left != null)result.right.left.parent = null;
            if(result.right.right != null)result.right.right.parent = null;
            root = merge(result.right.left, result.right.right);
            if(root != null) root.parent = null;
            update(root);
        }
    }
    ...
    boolean find(int x) {
        ...
        root = result.right;
        update(root);
        if(result.left != null && result.left.key == x) return true;
        return false;
    }
    ...
    long sum(int from, int to) {
        ...
        Vertex nLeft = merge(left,middle);
        root = merge(nLeft, right);
        return ans;
    }

    static List<Integer> inOrderList = new ArrayList<>();

    static void inOrder(Vertex root){
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (type) {
                ...
                case '-' : {
                    int x = nextInt();
                    int y = (x + last_sum_result) % MODULO;
                    erase(y);
                } break;
                case '?' : {
                    int x = nextInt();
